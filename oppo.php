...
<div id="poll" style="background-color: #c9c8c5;height: 1500px;font-size: 14px;">
  <div style="margin-right: 5%;margin-left: 5%;padding-top: 10px; height: 210px;">
    ...
    if($search)
    {
      ...
      $products = mysqli_query($conn, "SELECT * FROM `sanpham` WHERE `tensp` LIKE '%oppo%' ORDER BY `masp` ASC  LIMIT " . $item_per_page . " OFFSET " . $offset);
      $totalRecords = mysqli_query($conn, "SELECT * FROM `sanpham` WHERE `tensp` LIKE  '%oppo%' ");
